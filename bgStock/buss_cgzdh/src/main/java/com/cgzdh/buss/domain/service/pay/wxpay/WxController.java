...
import com.cgzdh.buss.util.HttpServRequestHelper;
...
public class WxController {
    ...
    public void notify(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if(WxController.LOGGER.isInfoEnabled()) {
            WxController.LOGGER.info("received WeChat notify,notify context is {}", notifyXml);
        }
        ...
        if(payResult == null){
            out.write(WxController.WX_RESPONSE_FAIL);
        } else {
            ...
            if(payResult.getTradeStatus()== PayTask.STATUS_SUCCESS){
                out.write(WxController.WX_RESPONSE_SUCCESS);
            }else {
                out.write(WxController.WX_RESPONSE_FAIL);
            }
        }
    ...
    private Trade buildPayResult(String notifyXml){
        ...
        if( map == null ){
            WxController.LOGGER.warn("can not parse the notify params,the notify xml={}", notifyXml);
            return null;
        }

        String sign = WxPayHelper.createSign(map);
        if( !sign.equalsIgnoreCase(map.get("sign")) ){
            WxController.LOGGER.warn("check params error,the notify xml is:" + notifyXml+";the sign of mine is:"+sign);
            return null;
        }
