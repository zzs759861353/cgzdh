...
public class ProcessorManager implements BeanPostProcessor {
    ...
    public ResultModel process(PayTask payTask)  {
        ...
        if(resolver == null){
            resolver = resolverMap.get(ProcessorManager.DEF_TASK_TYPE);
        }
        if(resolver == null){
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        ...
        if(bean instanceof ProcessorResolver){
            Class taskType = ProcessorManager.getGeneric(bean.getClass(),ProcessorResolver.class);
            if(taskType == null){
                taskType = ProcessorManager.DEF_TASK_TYPE;
            }
