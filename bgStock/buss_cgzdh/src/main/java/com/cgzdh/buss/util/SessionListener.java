...
public class SessionListener implements HttpSessionListener{
	 // key为sessionId，value为HttpSession，使用static，定义静态变量，使之程序运行时，一直存在内存中。    
	    ...
	    * 得到在线用户会话集合  
	    */    
	   public static List<HttpSession> geSessions() {    
	    ...
	    */    
	   public static Map<String, String> geSessionMap() {    
	       Map<String, String> map = new HashMap<String, String>();    
	       Iterator<String> iter = getSessionMapKeySetIt();    
	       while (iter.hasNext()) {    
	           String sessionId = iter.next();    
	           HttpSession session = getSessionMap().get(sessionId);    
	           TUserDto user = (TUserDto) session.getAttribute("LOCAL_CLINET_USER");    
	           if (user != null) {    
	   ...
	   public synchronized static void removeUserSession(String userId) {    
	       Map<String, String> userSessionMap = geSessionMap();    
	    ...
	    * @return  
	    */    
	   public synchronized static boolean checkIfHasLogin(TUserDto user) {    
	       Iterator<String> iter = getSessionMapKeySetIt();    
	       while (iter.hasNext()) {    
	           String sessionId = iter.next();    
	           HttpSession session = getSessionMap().get(sessionId);    
	           TUserDto sessionuser = (TUserDto) session.getAttribute("LOCAL_CLINET_USER");  // 这是你设置 保存用户对应session名   
	           if (sessionuser != null) {    
